...
// ---------------------------------------------------------------------------
// Definición de las tools del asistente IA.
// Cada tool mapea a un endpoint REST existente; la petición interna reenvía
// el Bearer token del admin, así requireAuth/requireTenant/requireRole y el
// aislamiento por school_id se aplican igual que en el panel.
// ---------------------------------------------------------------------------

export interface ToolRoute {
  method: 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE';
  path: string; // relativo a /api, ej. "/students"
...
interface ChatToolDef {
  tool: Anthropic.Tool;
  /** true = requiere confirmación humana antes de ejecutarse */
  sensitive?: boolean;
  /** Etiqueta humana para la UI ("Creando alumno…", tarjeta de confirmación) */
  label: (input: Record<string, any>) => string;
  route: (input: Record<string, any>, ctx: ToolContext) => ToolRoute;
}
...
const pick = (input: Record<string, any>, keys: string[]) => {
...
const queryOf = (input: Record<string, any>, keys: string[]) => {
  const out: Record<string, string> = {};
  for (const k of keys) {
    if (input[k] !== undefined && input[k] !== null) out[k] = String(input[k]);
  }
  return out;
};
...
const TOOL_DEFS: ChatToolDef[] = [
  // ------------------------------------------------------------------ consulta
  {
    tool: {
      ...
      description:
        'Obtiene el resumen general de la academia: alumnos activos/totales, ingresos del mes, pagos pendientes, próximos eventos y tasa de asistencia. Úsala para preguntas generales tipo "¿cómo va la academia?".',
  ...
  },
  {
    tool: {
      name: 'list_students',
      description:
        'Lista los alumnos de la academia, con su categoría y padre vinculado. Úsala para buscar alumnos por nombre (filtra tú sobre el resultado) o para obtener sus UUID antes de otras operaciones.',
      input_schema: {
        type: 'object',
  ...
  },
  {
    tool: {
      ...
      description:
        'Lista las categorías (equipos) de la academia con año de nacimiento, mensualidad y profesores asignados. Úsala siempre que necesites un category_id.',
      input_schema: { type: 'object', properties: {}, required: [] },
    },
  ...
  },
  {
    tool: {
      ...
      description:
        'Lista los alumnos con mensualidad pendiente de pago en un mes. Úsala para "¿quién debe?", "morosos", "pagos pendientes".',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      name: 'list_users',
      description:
        'Lista los usuarios de la academia (profesores, padres, admins, alumnos-usuario). Úsala para obtener UUIDs de profesores o padres.',
      input_schema: {
        type: 'object',
  ...
  // ----------------------------------------------------------- escritura directa
  {
    tool: {
      name: 'create_student',
      description:
        'Crea un alumno nuevo en la academia. Si no conoces el category_id, usa list_categories primero. Devuelve también el username y contraseña temporal generados.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      name: 'update_student',
      description:
        'Actualiza datos de un alumno existente (nombre, fecha de nacimiento, categoría, teléfono, dirección, contacto de emergencia, notas médicas). Solo envía los campos a cambiar.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      ...
      description:
        'Crea una categoría completa CON horario de entrenamiento recurrente (días, hora, sede) y genera los entrenamientos. Prefierela sobre create_category cuando el admin indique horario.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      name: 'create_event',
      description:
        'Crea un evento (entrenamiento, partido, torneo) para una categoría. Puede ser recurrente semanal con recurringWeeks.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      ...
      description:
        'Cancela UNA sesión de entrenamiento concreta (no borra el evento). Pasa training_id, o event_id + date.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      ...
      description:
        'Registra la asistencia de una categoría en una fecha. records es la lista completa de alumnos con present true/false (reemplaza lo previo de ese día).',
      input_schema: {
        type: 'object',
        properties: {
          category_id: str('UUID de la categoría'),
  ...
  // ------------------------------------------------- sensibles (confirmación)
  {
    tool: {
      name: 'delete_student',
      description:
        'ELIMINA un alumno de la academia (se archiva en la papelera de eliminados). Acción destructiva: el usuario deberá confirmarla.',
      input_schema: {
        type: 'object',
  ...
  },
  {
    tool: {
      ...
      description:
        'Registra un pago de mensualidad u otro concepto de uno o varios alumnos. Acción sensible: el usuario deberá confirmarla. Notifica a los padres.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      name: 'invite_teacher',
      description:
        'Invita a un profesor nuevo por email (recibe un correo de invitación). Acción sensible: el usuario deberá confirmarla.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  },
  {
    tool: {
      name: 'invite_admin',
      description:
        'Crea otro administrador de la academia con contraseña temporal. Acción sensible: el usuario deberá confirmarla.',
      input_schema: {
        type: 'object',
        properties: {
...
export const CHAT_TOOLS: Anthropic.Tool[] = TOOL_DEFS.map((d) => d.tool);

export const SENSITIVE_TOOLS = new Set(TOOL_DEFS.filter((d) => d.sensitive).map((d) => d.tool.name));

export function toolRoute(name: string, input: Record<string, any>, ctx: ToolContext): ToolRoute | null {
