...
export const exportPayments = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error) return res.status(500).json({ error: 'Error al exportar pagos.' });

    const monthNames = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const csvHeader = 'Fecha,Tipo,Alumno/Profesor,Monto,Descripción\n';
    const csvRows = (payments || []).map((p: any) =>
      `${p.payment_date},${p.payment_type === 'mensualidad' ? 'Mensualidad' : 'Pago Profesor'},${p.student?.full_name || p.teacher?.full_name || '—'},$${p.amount},${p.description || ''}`
    ).join('\n');
...
export const exportAttendance = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (category_id && attendances) {
      attendances.filter((a: any) => a.student?.category?.id === category_id);
    }

    if (error) return res.status(500).json({ error: 'Error al exportar asistencias.' });

    const monthNames = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const csvHeader = 'Fecha,Alumno,Categoría,Asistió\n';
    const csvRows = (attendances || []).map((a: any) =>
      `${a.date},${a.student?.full_name || '—'},${a.student?.category?.name || '—'},${a.present ? 'Sí' : 'No'}`
    ).join('\n');
