...
const createVenue = async (req, res) => {
    const { school_id } = req.tenant;
    const { name, address, notes, is_external, status, surface_type, capacity, has_lighting, is_covered, type_label, latitude, longitude } = req.body;
    if (!name)
...
const deleteVenue = async (req, res) => {
    ...
    try {
        const { error } = await supabase_1.supabaseAdmin
            ...
            .eq('school_id', school_id);
        if (error)
            return res.status(400).json({ error: 'No se pudo eliminar la sede.' });
