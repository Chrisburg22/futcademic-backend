...
exports.saveAttendances = exports.markTrainingComplete = exports.getAttendancesByStudent = exports.getAttendancesByCategory = void 0;
const supabase_1 = require("../config/supabase");
const getAttendancesByCategory = async (req, res) => {
...
exports.getAttendancesByCategory = getAttendancesByCategory;
...
exports.getAttendancesByStudent = getAttendancesByStudent;
const markTrainingComplete = async (req, res) => {
    const { school_id } = req.tenant;
    const { id } = req.params;
    const { data, error } = await supabase_1.supabaseAdmin
        .from('trainings')
...
exports.markTrainingComplete = markTrainingComplete;
const saveAttendances = async (req, res) => {
    ...
    try {
        ...
        if (finalTrainingId) {
            ...
            const { data: trExists } = await supabase_1.supabaseAdmin
                .from('trainings')
                .select('id')
            ...
            if (!trExists) {
                ...
                const { data: trFound } = await supabase_1.supabaseAdmin
                    .from('trainings')
                    .select('id')
        ...
        const attendanceToInsert = records.map((r) => {
        ...
        if (finalTrainingId) {
            const { error: trError } = await supabase_1.supabaseAdmin
                ...
                .eq('school_id', school_id);
            if (trError)
                console.error('[ATTENDANCE] Error updating training status:', trError);
        }
...
exports.saveAttendances = saveAttendances;
