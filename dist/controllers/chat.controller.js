...
exports.resolveConfirmation = exports.sendMessage = exports.deleteConversation = exports.getMessages = exports.listConversations = void 0;
const supabase_1 = require("../config/supabase");
const sse_1 = require("../services/chat/sse");
const agent_1 = require("../services/chat/agent");
const executor_1 = require("../services/chat/executor");
const tools_1 = require("../services/chat/tools");
const store_1 = require("../services/chat/store");
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function findConversation(id, schoolId, userId) {
...
async function getSchoolName(schoolId) {
    const { data } = await supabase_1.supabaseAdmin.from('schools').select('name').eq('id', schoolId).single();
    return data?.name || 'la academia';
}
function deniedResult(toolUseId, reason) {
...
const listConversations = async (req, res) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            ...
            .limit(50);
        if (error)
            return res.status(500).json({ error: 'Error listando conversaciones.', details: error.message });
        res.json(data);
    }
    catch (err) {
...
const getMessages = async (req, res) => {
    try {
        ...
        if (error)
            return res.status(500).json({ error: 'Error cargando mensajes.', details: error.message });
        res.json({
            conversation: {
                id: conversation.id,
...
const deleteConversation = async (req, res) => {
    try {
        ...
        if (!conversation)
            return res.status(404).json({ error: 'Conversación no encontrada.' });
        const { error } = await supabase_1.supabaseAdmin.from('chat_conversations').delete().eq('id', conversation.id);
        if (error)
            return res.status(500).json({ error: 'Error eliminando conversación.', details: error.message });
...
const sendMessage = async (req, res) => {
    ...
    try {
        ...
        if (conversation.status === 'awaiting_confirmation' && conversation.pending_state) {
            const pending = conversation.pending_state;
            const denied = pending.pending.map((p) => deniedResult(p.tool_use_id, 'El usuario envió otro mensaje sin confirmar; la acción quedó cancelada.'));
            await (0, store_1.saveMessage)(conversation.id, 'user', [...pending.executed_results, ...denied]);
            await (0, store_1.clearPending)(conversation.id);
        }
        await (0, store_1.saveMessage)(conversation.id, 'user', [{ type: 'text', text: message }]);
        const messages = await (0, store_1.loadHistory)(conversation.id);
        const schoolName = await getSchoolName(school_id);
        await (0, agent_1.runAgentTurn)({
 ...
 */
const resolveConfirmation = async (req, res) => {
    const { school_id, user_id, role } = req.tenant;
    ...
    sse.send('conversation', { id: conversation.id, title: conversation.title });
    try {
        const results = [...(pending.executed_results || [])];
        for (const p of pending.pending) {
            if (approved) {
                sse.send('tool_start', { name: p.name, label: (0, tools_1.toolLabel)(p.name, p.input) });
                const r = await (0, executor_1.executeTool)(p.name, p.input, req.headers.authorization, { schoolId: school_id });
        ...
        await (0, store_1.saveMessage)(conversation.id, 'user', results);
        await (0, store_1.clearPending)(conversation.id);
        const messages = await (0, store_1.loadHistory)(conversation.id);
        const schoolName = await getSchoolName(school_id);
        await (0, agent_1.runAgentTurn)({
