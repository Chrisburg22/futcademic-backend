...
describe('TC-I-01 — CRUD completo alumno', () => {
    ...
    function mockStudentCreation() {
        ...
        const usersBuilder = new helpers_1.MockQueryBuilder({ data: null, error: null });
        ...
        // students insert
        const studentsBuilder = new helpers_1.MockQueryBuilder({ data: STUDENT, error: null });
    ...
    it('PATCH /api/students/:id/status — cambia estatus (200)', async () => {
        setupAuthMocks();
        const studentsBuilder = new helpers_1.MockQueryBuilder({ data: { ...STUDENT, status: 'becado' }, error: null });
...
describe('TC-I-03 — Pase de lista batch', () => {
    it('guarda asistencia, marca training completado y responde < 500ms', async () => {
        ...
        const trainingsBuilder = new helpers_1.MockQueryBuilder({ data: { id: TRAINING_ID }, error: null });
...
describe('TC-I-04 — Evento recurrente y cancelación de sesión', () => {
    ...
    it('crea evento con 8 sesiones (201)', async () => {
        ...
        // events insert
        const eventsBuilder = new helpers_1.MockQueryBuilder({ data: { id: EVENT_ID, name: 'Entreno Sub-12' }, error: null });
        ...
        // trainings insert
        const trainingsBuilder = new helpers_1.MockQueryBuilder({ data: trainingsInserted, error: null });
        ...
        expect(trainingsBuilder.insert).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ event_id: EVENT_ID })]));
    });
    it('cancela sesión específica → is_cancelled=true (200)', async () => {
        ...
        const trainingsBuilder = new helpers_1.MockQueryBuilder({ data: trainingData, error: null });
        ...
        expect(trainingsBuilder.update).toHaveBeenCalledWith(expect.objectContaining({ is_cancelled: true }));
    });
});
// ─────────────────────────────────────────────────────────────────────────────
...
describe('TC-I-05 — Venue CRUD + asignación a evento', () => {
    ...
    it('crea venue (201)', async () => {
        setupAuthMocks();
        const venuesBuilder = new helpers_1.MockQueryBuilder({ data: VENUE, error: null });
    ...
    it('crea evento con venue_id y venue_name visible en GET (201 + 200)', async () => {
        ...
        // events create
        const eventsBuilder = new helpers_1.MockQueryBuilder({ data: [EVENT_WITH_VENUE], error: null });
        eventsBuilder.insert = jest.fn().mockReturnThis();
        eventsBuilder.select = jest.fn().mockReturnThis();
        eventsBuilder.single = jest.fn().mockResolvedValue({ data: EVENT_WITH_VENUE, error: null });
        mockSupabase._builders.set('events', eventsBuilder);
        // trainings insert
        const trainingsBuilder = new helpers_1.MockQueryBuilder({ data: [], error: null });
        ...
        const events = getRes.body;
        const evWithVenue = events.find((e) => e.venue_id === VENUE_ID || e.venue?.name === 'Campo A');
