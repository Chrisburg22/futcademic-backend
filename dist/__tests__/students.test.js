...
describe('getStudentStats', () => {
    it('returns student stats with streak and attendance', async () => {
        ...
        const req = (0, helpers_1.mockReq)({ params: { id: 'student-1' } });
        const res = (0, helpers_1.mockRes)();
        await (0, student_controller_1.getStudentStats)(req, res);
...
describe('getStudentTeam', () => {
    it('returns teammates from same category', async () => {
        ...
        const req = (0, helpers_1.mockReq)({ params: { id: 'student-1' } });
        const res = (0, helpers_1.mockRes)();
        await (0, student_controller_1.getStudentTeam)(req, res);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
            teamName: 'U-12',
            totalTeammates: 2,
        }));
...
describe('updateStatus', () => {
    it('validates new status values', async () => {
        ...
        const req = (0, helpers_1.mockReq)({ params: { id: 'student-1' }, body: { status: 'pendiente_pago' } });
        const res = (0, helpers_1.mockRes)();
        await (0, student_controller_1.updateStatus)(req, res);
        expect(res.json).toHaveBeenCalledWith({ message: 'Estado actualizado.' });
    });
    it('rejects invalid status values', async () => {
        const req = (0, helpers_1.mockReq)({ body: { status: 'invalid' } });
        const res = (0, helpers_1.mockRes)();
        await (0, student_controller_1.updateStatus)(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
    });
    it('rejects old status values', async () => {
        const req = (0, helpers_1.mockReq)({ body: { status: 'beca' } });
        const res = (0, helpers_1.mockRes)();
        await (0, student_controller_1.updateStatus)(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
    });
